...
internal class TestBed
{
    ...
    public Mock<ITMLinkCharacteristic> NotifyMessageCharacteristic { get; set; } = new Mock<ITMLinkCharacteristic>(MockBehavior.Strict);
    ...
    internal void SendDelayedNotification(V1.CommandType command, int notificationTimeSpan)
    {
        Task.Delay(notificationTimeSpan).ContinueWith(
            task =>
            {
                ...
                NotifyMessageCharacteristic.Raise(m => m.CharacteristicValueChanged -= null, null, new ValueChangedEventArgs { Value = NotifyBytes });
            });
    }
}
